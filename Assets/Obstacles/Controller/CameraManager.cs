	...
	void HandleRotations(float d, float v, float h, float targetSpeed)
{
		...
		// Invert the vertical input if necessary (commonly done)
		tiltAngle -= smoothY * targetSpeed; // Up/Down control
		tiltAngle = Mathf.Clamp(tiltAngle, minAngle, maxAngle);
		pivot.localRotation = Quaternion.Euler(tiltAngle, 0, 0); // Apply the tilt to the pivot

		lookAngle += smoothX * targetSpeed; // Left/Right control
		transform.rotation = Quaternion.Euler(0, lookAngle, 0); // Apply yaw
	}
