...
public class CharacterControls : MonoBehaviour {
	...
	void  Start (){
		...
		scoreBoardManager = FindObjectOfType<ScoreBoardManager>(); // Get the ScoreBoardManager
	}
	
	bool IsGrounded (){
		return Physics.Raycast(transform.position, -Vector3.up, distToGround + 0.1f);
	}
	private int playerIndex; // Add this variable

	public void SetPlayerIndex(int index)
	{
		playerIndex = index; // Assign the player index
	}

	public ParticleSystem destructionEffect; // Assign your particle effect prefab in the Inspector
	...
	private void OnCollisionEnter(Collision collision)
	{
		if (collision.gameObject.CompareTag("Obstacle"))
		{
			...
			// {
			// 	ParticleSystem effect = Instantiate(destructionEffect, collision.transform.position, Quaternion.identity);
			// 	effect.Play();
			// 	Destroy(effect.gameObject, effect.main.duration); // Destroy the particle effect after it finishes
			// }
	...
	// Debug.Log($"Player {playerIndex}");
		float h = Input.GetAxis("Horizontal" + playerIndex); // Use player index
    	float v = Input.GetAxis("Vertical" + playerIndex); // Use player index
		// Debug.Log($"Player {playerIndex}: Horizontal Input = {h}, Vertical Input = {v}");
